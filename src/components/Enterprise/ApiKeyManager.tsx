...
const STATUS_CONFIG: Record<ApiKeyStatus, { label: string; color: string; dot: string; icon: React.ReactNode }> = {
  ACTIVE: { label: 'Active', color: 'bg-emerald-50 text-emerald-700 border-emerald-200', dot: 'bg-emerald-500', icon: <CheckCircle2 className="h-3.5 w-3.5" /> },
  REVOKED: { label: 'Revoked', color: 'bg-slate-100 text-slate-600 border-slate-200', dot: 'bg-slate-400', icon: <XCircle className="h-3.5 w-3.5" /> },
  EXPIRED: { label: 'Expired', color: 'bg-amber-50 text-amber-700 border-amber-200', dot: 'bg-amber-500', icon: <Clock className="h-3.5 w-3.5" /> },
  SUSPENDED: { label: 'Suspended', color: 'bg-red-50 text-red-700 border-red-200', dot: 'bg-red-500', icon: <AlertTriangle className="h-3.5 w-3.5" /> },
  PENDING_ROTATION: { label: 'Rotation Due', color: 'bg-purple-50 text-purple-700 border-purple-200', dot: 'bg-purple-500', icon: <RefreshCw className="h-3.5 w-3.5" /> },
};
...
const SCOPE_COLORS: Record<ApiKeyScope, string> = {
  ...
  WEBHOOK_ONLY: 'bg-orange-50 text-orange-700 border-orange-200',
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest">API Keys ({filteredKeys.length})</h3>
        <div className="flex items-center gap-2">
          ...
          <button className="flex items-center gap-1.5 px-4 py-1.5 bg-slate-900 text-white rounded-lg text-xs font-bold hover:bg-indigo-700 transition-colors">
      ...
      {showFilters && (
        <div className="flex items-center gap-3 flex-wrap">
          <select
            ...
            className="px-3 py-1.5 text-xs font-bold bg-white border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500"
          >
          ...
          return (
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h4 className="text-sm font-bold text-slate-900 truncate">{key.name}</h4>
                ...
                <div className="hidden md:flex items-center gap-6 shrink-0">
                  <div className="text-right">
                    <p className="text-sm font-bold text-slate-900">{formatNumber(key.totalRequests)}</p>
                    <p className="text-[10px] text-slate-400 font-bold uppercase">Requests</p>
                  </div>
                  <div className="text-right">
                    <p className={`text-sm font-bold ${key.errorRate > 2 ? 'text-red-600' : key.errorRate > 1 ? 'text-amber-600' : 'text-slate-900'}`}>
                      {key.errorRate}%
                    </p>
                    <p className="text-[10px] text-slate-400 font-bold uppercase">Error Rate</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-bold text-slate-900">{key.avgLatencyMs}ms</p>
              ...
              {key.rotationDueAt && key.status === 'ACTIVE' && (
                <div className="mt-3 px-3 py-2 bg-purple-50 rounded-lg border border-purple-100 flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 mt-3">
                {key.tags.map(tag => (
                  <span key={tag} className="px-2 py-0.5 bg-slate-50 text-slate-500 rounded text-[10px] font-bold border border-slate-100">
                    {tag}
                  </span>
                ))}
