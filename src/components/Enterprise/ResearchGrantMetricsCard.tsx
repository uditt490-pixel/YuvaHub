      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm">
          ...
          <div className="mt-3 flex items-baseline gap-2">
            ...
            <span className="text-xs font-semibold text-slate-500">/ ₹{(analytics.totalRequestedCapitalLakhs / 100).toFixed(2)} Cr Req</span>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm">
          ...
          <div className="mt-3 flex items-baseline gap-2">
            ...
            <span className="text-xs font-semibold text-slate-500">/ {analytics.activeProposals} Active</span>
          </div>
          <div className="mt-2 text-xs font-medium text-blue-700 dark:text-blue-300 flex items-center gap-1">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
          ...
          <div className="space-y-3">
            {analytics.categoryDistribution.map((item) => (
              <div key={item.category} className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800">
                <div className="flex justify-between text-xs font-semibold mb-1">
                  <span className="text-slate-800 dark:text-slate-200">{item.category.replace(/_/g, ' ')}</span>
                  <span className="text-emerald-600 dark:text-emerald-400 font-bold">₹{(item.capitalLakhs / 100).toFixed(1)} Cr ({item.proposalCount} Labs)</span>
                </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
          ...
          <div className="space-y-3">
            {analytics.institutionAllocations.map((inst) => (
              <div key={inst.college} className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800">
                <div className="flex justify-between text-xs font-semibold mb-1">
                  <span className="text-slate-800 dark:text-slate-200">{inst.college}</span>
                  <span className="text-indigo-600 dark:text-indigo-400 font-bold">₹{(inst.awardedCapitalLakhs / 100).toFixed(1)} Cr ({inst.projectCount} Grants)</span>
                </div>
                <div className="w-full bg-slate-200 dark:bg-slate-700 h-2 rounded-full overflow-hidden">
                  <div className="bg-indigo-500 h-full rounded-full" style={{ width: `${Math.min(100, inst.awardedCapitalLakhs / 10)}%` }} />
                </div>
              </div>
            ))}
          </div>
