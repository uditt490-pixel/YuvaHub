...
const SEV_COLORS = { P0: 'bg-red-500', P1: 'bg-orange-500', P2: 'bg-amber-500', P3: 'bg-blue-500' };
const RULE_ACTION_COLORS: Record<string, string> = { block: 'bg-red-100 text-red-700', flag: 'bg-amber-100 text-amber-700', challenge: 'bg-orange-100 text-orange-700', monitor: 'bg-blue-100 text-blue-700', notify: 'bg-slate-100 text-slate-600' };
            ...
            {openAlertCount > 0 && (
              <div className="bg-gradient-to-r from-red-50 to-orange-50 rounded-2xl border border-red-200 p-6">
                <div className="flex items-center gap-3 mb-3">
                  <div className="p-2 bg-red-100 rounded-xl"><Ban className="h-5 w-5 text-red-600" /></div>
                  <div>
                ...
                <div className="space-y-2">
                  {openAlerts.slice(0, 3).map(alert => (
                    <div key={alert.id} className="flex items-center gap-3 px-4 py-2.5 bg-white/80 rounded-xl border border-red-100">
                      <div className={`w-2 h-2 rounded-full ${SEV_COLORS[alert.severity]}`} />
                      <span className="text-sm font-medium text-slate-800 truncate flex-1">{alert.title}</span>
                      <span className="text-xs text-slate-500">Risk: {alert.riskScore.toFixed(0)}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-800">Detection Rules</h3>
                  <p className="text-xs text-slate-500">{rules.length} rules · {rules.filter(r => r.enabled).length} active</p>
                </div>
              </div>
            </div>
            <div className="divide-y divide-slate-50">
              {isLoading ? (
                Array.from({ length: 5 }).map((_, i) => <div key={i} className="px-6 py-4 animate-pulse"><div className="h-4 bg-slate-200 rounded w-full" /></div>)
              ) : rules.map(rule => (
                <div key={rule.id} className="px-6 py-4 hover:bg-slate-50 transition-colors">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-right flex-shrink-0 hidden sm:block">
                      <div className="text-sm font-mono font-bold text-slate-700">{rule.triggerCount}</div>
                      <div className="text-[10px] text-slate-400">triggers</div>
                    </div>
                    <div className="text-right flex-shrink-0 hidden sm:block">
                      <div className={`text-sm font-mono font-bold ${rule.falsePositiveRate > 10 ? 'text-red-600' : 'text-slate-700'}`}>{rule.falsePositiveRate}%</div>
                      <div className="text-[10px] text-slate-400">FP rate</div>
                    </div>
                    <div className="text-right flex-shrink-0 hidden md:block">
                      <div className="text-xs text-slate-500">{rule.lastTriggered ? new Date(rule.lastTriggered).toLocaleDateString() : 'Never'}</div>
                      <div className="text-[10px] text-slate-400">last fired</div>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-800">Fraud Alerts</h3>
                  <p className="text-xs text-slate-500">{alerts.length} total · {openAlertCount} open</p>
                </div>
              </div>
            </div>
            <div className="divide-y divide-slate-50 max-h-[700px] overflow-y-auto">
              {isLoading ? (
                Array.from({ length: 6 }).map((_, i) => <div key={i} className="px-6 py-4 animate-pulse"><div className="h-4 bg-slate-200 rounded w-full" /></div>)
              ) : alerts.map(alert => (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold capitalize ${
                          alert.status === 'open' ? 'bg-red-100 text-red-700' : alert.status === 'investigating' ? 'bg-amber-100 text-amber-700' : alert.status === 'resolved' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'
                        }`}>{alert.status}</span>
                      </div>
                      ...
                      {alert.resolution && <div className="mt-2 px-3 py-2 bg-emerald-50 rounded-lg text-xs text-emerald-700">✓ {alert.resolution}</div>}
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="px-2.5 py-1 text-xs font-bold uppercase tracking-wider text-red-700 bg-red-100 rounded-full flex items-center gap-1"><ShieldAlert className="h-3 w-3" /> Fraud Detection Active</span>
              </div>
              <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Fraud Detection & Risk Intelligence</h1>
              <p className="text-slate-500 mt-1">Real-time transaction monitoring, risk scoring, and threat intelligence.</p>
            </div>
            <div className="flex items-center gap-3">
              <button onClick={() => setAutoRefresh(!autoRefresh)}
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium border transition-all ${autoRefresh ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                <div className={`w-2 h-2 rounded-full ${autoRefresh ? 'bg-indigo-500 animate-pulse' : 'bg-slate-300'}`} />
                Auto-refresh {autoRefresh ? 'ON' : 'OFF'}
              </button>
              <button onClick={loadData} disabled={isLoading}
                className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl hover:bg-slate-50 transition-all font-medium text-sm shadow-sm disabled:opacity-50">
            ...
            {TABS.map(tab => {
              ...
              return (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-all whitespace-nowrap ${isActive ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-800'}`}>
                  {tab.icon}{tab.label}
