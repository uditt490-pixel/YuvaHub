...
import { TraumaPatient, TraumaAlert } from "../../../types/traumaTelemetry";

interface TraumaAlertConsoleModalProps {
  isOpen: boolean;
  onClose: () => void;
  patients: TraumaPatient[];
  onAcknowledgeAlert: (patientId: string, alertId: string, clinicianName: string) => void;
}

export const TraumaAlertConsoleModal: React.FC<TraumaAlertConsoleModalProps> = ({
  isOpen,
  onClose,
  patients,
  onAcknowledgeAlert
}) => {
  const [clinicianName, setClinicianName] = useState("Dr. Trauma Attending, MD");
  const [filterSeverity, setFilterSeverity] = useState<string>("ALL");

  if (!isOpen) return null;

  // Flatten all alerts across patients
  const allAlerts: Array<TraumaAlert & { patientName: string; bay: string; mrn: string }> = [];
  patients.forEach((p) => {
    p.activeAlerts.forEach((a) => {
      allAlerts.push({
        ...a,
        patientName: p.name,
        bay: p.traumaBayNumber,
        mrn: p.mrn
      });
    });
  });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-4xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] text-slate-100">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Multi-Patient Trauma Bay Surveillance • Lethal Triad Alarms • REBOA Ischemia Limits • Viscoelastic Coagulopathy
              </p>
            </div>
          </div>
        ...
        {/* Controls & Clinician Sign-Off Bar */}
        <div className="p-3.5 bg-slate-900/90 border-b border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <select
              ...
              className="bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-slate-200 focus:outline-none focus:border-rose-500"
            >
              <option value="ALL">All Severity Levels</option>
              <option value="CRITICAL_STAT">Critical STAT Only</option>
              <option value="HIGH_WARNING">High Warning Only</option>
              <option value="MODERATE_ALERT">Moderate Alert Only</option>
            </select>
          </div>

          <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
            <User className="w-4 h-4 text-cyan-400" />
            <span className="text-slate-400">Attending Sign-Off:</span>
            <input
              type="text"
              value={clinicianName}
        ...
        <div className="p-4 overflow-y-auto space-y-3 flex-1">
          {filteredAlerts.length === 0 ? (
            <div className="text-center py-12 text-slate-400">
              ...
              <p className="text-sm font-bold text-slate-300">All Clinical Parameters Within Target Ranges</p>
              <p className="text-xs text-slate-500 mt-1">No active unacknowledged physiological alarms.</p>
            </div>
          ) : (
                    ...
                    <div className="space-y-1.5 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="font-mono text-xs font-bold text-cyan-300">
                          {alert.bay} • {alert.patientName} ({alert.mrn})
                        </span>
                      ...
                      <p className="text-xs text-amber-300/90 bg-amber-950/40 p-2 rounded border border-amber-800/40">
                        <strong className="text-amber-400 font-bold">Suggested Escalation: </strong>
        ...
        <div className="p-3 bg-slate-950 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400">
          <span>Audited Clinical Event Log (FDA 21 CFR Part 11 Compliant)</span>
          <button
            onClick={onClose}
