...
const STATUS_COLORS: Record<string, { bg: string; text: string; border: string }> = {
  ...
  CONFIGURING: { bg: 'bg-amber-100', text: 'text-amber-700', border: 'border-amber-200' },
  ERROR: { bg: 'bg-red-100', text: 'text-red-700', border: 'border-red-200' },
  MAINTENANCE: { bg: 'bg-purple-100', text: 'text-purple-700', border: 'border-purple-200' },
  EXPIRED: { bg: 'bg-slate-100', text: 'text-slate-500', border: 'border-slate-200' },
  REVOKED: { bg: 'bg-red-100', text: 'text-red-600', border: 'border-red-200' },
  IDLE_TIMEOUT: { bg: 'bg-amber-100', text: 'text-amber-600', border: 'border-amber-200' },
  CONCURRENT_LIMIT: { bg: 'bg-orange-100', text: 'text-orange-600', border: 'border-orange-200' },
  ENABLED: { bg: 'bg-emerald-100', text: 'text-emerald-700', border: 'border-emerald-200' },
  DISABLED: { bg: 'bg-slate-100', text: 'text-slate-500', border: 'border-slate-200' },
  ENFORCED: { bg: 'bg-blue-100', text: 'text-blue-700', border: 'border-blue-200' },
  SETUP_INCOMPLETE: { bg: 'bg-amber-100', text: 'text-amber-700', border: 'border-amber-200' },
  ...
  PENDING_VERIFICATION: { bg: 'bg-amber-100', text: 'text-amber-700', border: 'border-amber-200' },
  LOCKED: { bg: 'bg-red-100', text: 'text-red-700', border: 'border-red-200' },
  DEACTIVATED: { bg: 'bg-slate-100', text: 'text-slate-500', border: 'border-slate-200' },
...
const OverviewPanel: React.FC<{ metrics: IdentityMetrics | null; isLoading: boolean }> = ({ metrics, isLoading }) => {
  if (isLoading || !metrics) return <div className="space-y-4">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-20 bg-white rounded-xl border border-slate-200 animate-pulse" />)}</div>;

  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: 'Active Sessions', value: metrics.activeSessions, color: 'bg-indigo-500' },
          { label: 'SSO Users', value: metrics.ssoUsers, color: 'bg-emerald-500' },
          { label: 'MFA Enforced', value: metrics.mfaEnforced, color: 'bg-blue-500' },
          { label: 'Failed Logins (24h)', value: metrics.failedLogins24h, color: 'bg-red-500' },
        ].map(kpi => (
          <div key={kpi.label} className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-4">SSO Provider Usage</h4>
          <div className="space-y-2.5">
            {metrics.providerBreakdown.map(p => (
              <div key={p.provider} className="flex items-center gap-3">
                <span className="text-[11px] font-bold text-slate-600 w-32 truncate">{p.provider.replace(/_/g, ' ')}</span>
                <div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-indigo-500 rounded-full" style={{ width: `${p.percentage}%` }} /></div>
                <span className="text-xs font-bold text-slate-700 w-12 text-right">{p.percentage}%</span>
              </div>
            ))}
          </div>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-4">Auth Method Distribution</h4>
          <div className="space-y-2.5">
            {metrics.authMethodBreakdown.map(a => (
              <div key={a.method} className="flex items-center gap-3">
                <span className="text-[11px] font-bold text-slate-600 w-32">{a.method}</span>
                <div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-emerald-500 rounded-full" style={{ width: `${a.percentage}%` }} /></div>
                <span className="text-xs font-bold text-slate-700 w-12 text-right">{a.percentage}%</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-4">Session Risk Distribution</h4>
          <div className="space-y-3">
            {metrics.riskDistribution.map(r => (
              <div key={r.level} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-slate-500 rounded-full" style={{ width: `${Math.max(r.percentage, 2)}%` }} /></div>
                <span className="text-xs font-bold text-slate-700 w-12 text-right">{r.count}</span>
              </div>
            ))}
          </div>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-4">Geographic Distribution</h4>
          <div className="space-y-3">
            {metrics.geoDistribution.map(g => (
              <div key={g.country} className="flex items-center gap-3">
                <span className="text-[11px] font-bold text-slate-600 w-28">{g.country}</span>
                <div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-blue-500 rounded-full" style={{ width: `${g.percentage}%` }} /></div>
                <span className="text-xs font-bold text-slate-700 w-12 text-right">{g.sessions}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Top Risky Users */}
      <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
        <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-4">Top Risky Users</h4>
        <div className="space-y-3">
          {metrics.topRiskyUsers.map((u, i) => (
            <div key={i} className="flex items-center gap-3 p-3 rounded-lg hover:bg-slate-50 transition-colors">
              ...
              <div className="flex-1 min-w-0"><p className="text-xs font-bold text-slate-800">{u.user}</p><p className="text-[10px] text-slate-500">{u.email}</p></div>
              <span className="text-[10px] text-slate-400">{u.location}</span>
              <div className="text-right"><p className="text-sm font-bold text-red-600">{u.riskScore}</p><p className="text-[10px] text-slate-400">{u.sessions} sessions</p></div>
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value as SessionStatus | 'ALL')}
          className="px-3 py-2.5 text-xs font-bold bg-white border border-slate-200 rounded-xl outline-none">
      ...
      <div className="space-y-2">
        {filtered.map(session => {
          ...
          return (
            <div key={session.id} className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition-all">
              <div className="flex items-center gap-3 p-4 cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : session.id)}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <h4 className="text-sm font-bold text-slate-800 truncate">{session.userName}</h4>
                  ...
                  <p className="text-xs text-slate-500 truncate">{session.userEmail}</p>
              ...
              {isExpanded && (
                <div className="px-4 pb-4 pt-2 border-t border-slate-100 space-y-3 animate-in slide-in-from-top-1 duration-200">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div className="bg-slate-50 rounded-lg p-3"><span className="text-[10px] font-bold text-slate-400 uppercase">IP Address</span><p className="text-xs font-mono font-bold text-slate-800 mt-0.5">{session.ipAddress}</p></div>
                    <div className="bg-slate-50 rounded-lg p-3"><span className="text-[10px] font-bold text-slate-400 uppercase">Location</span><p className="text-xs font-bold text-slate-800 mt-0.5">{session.geoLocation.city}, {session.geoLocation.country}</p></div>
                    <div className="bg-slate-50 rounded-lg p-3"><span className="text-[10px] font-bold text-slate-400 uppercase">SSO Provider</span><p className="text-xs font-bold text-slate-800 mt-0.5">{session.ssoProvider || 'Direct'}</p></div>
                    <div className="bg-slate-50 rounded-lg p-3"><span className="text-[10px] font-bold text-slate-400 uppercase">MFA Verified</span><p className={`text-xs font-bold mt-0.5 ${session.mfaVerified ? 'text-emerald-600' : 'text-red-600'}`}>{session.mfaVerified ? 'Yes' : 'No'}</p></div>
                  </div>
                  {session.riskFactors.length > 0 && (
                    <div>
                      <span className="text-[10px] font-bold text-slate-400 uppercase mb-1.5 block">Risk Factors</span>
                  ...
                  {session.status === 'ACTIVE' && (
                    <button onClick={(e) => { e.stopPropagation(); onRevoke(session.id); }}
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 text-red-700 rounded-lg text-[11px] font-bold border border-red-200 hover:bg-red-100 transition-colors">
...
const ProvidersPanel: React.FC<{ providers: SsoProvider[]; isLoading: boolean }> = ({ providers, isLoading }) => {
  if (isLoading) return <div className="space-y-3">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-28 bg-white rounded-xl border border-slate-200 animate-pulse" />)}</div>;
  ...
  return (
    <div className="space-y-3">
      {providers.map(p => {
        ...
        return (
          <div key={p.id} className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-all">
  ...
  return (
    <div className="space-y-3">
      {configs.map(cfg => {
        ...
        return (
          <div key={cfg.id} className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm hover:shadow-md transition-all">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <h4 className="text-sm font-bold text-slate-800">{cfg.userName}</h4>
                ...
                <p className="text-xs text-slate-500">{cfg.userEmail}</p>
...
const AuditPanel: React.FC<{ entries: IdentityAuditEntry[]; isLoading: boolean }> = ({ entries, isLoading }) => {
  ...
  if (isLoading) return <div className="space-y-3">{Array.from({ length: 5 }).map((_, i) => <div key={i} className="h-16 bg-white rounded-xl border border-slate-200 animate-pulse" />)}</div>;
        ...
        {filtered.slice(0, 30).map(entry => {
          ...
          return (
            <div key={entry.id} className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <span className="text-xs font-bold text-slate-800">{entry.eventType.replace(/_/g, ' ')}</span>
              ...
              {entry.riskScore !== undefined && (
                <span className={`text-xs font-bold ${entry.riskScore > 60 ? 'text-red-600' : entry.riskScore > 30 ? 'text-amber-600' : 'text-emerald-600'}`}>{entry.riskScore}</span>
              )}
            </div>
          );
        })}
      ...
      <div className="max-w-[1500px] mx-auto space-y-6">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
          <div>
            ...
            <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">SSO & Identity Management</h1>
            <p className="text-sm text-slate-500 mt-2 max-w-xl">Single sign-on providers, session management, MFA enforcement, access policies, and identity audit trail.</p>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm"><span className="text-[10px] font-bold text-slate-500 uppercase">SSO Providers</span><p className="text-2xl font-black text-slate-900 mt-1">{metrics?.ssoProviders ?? '...'}</p><span className="text-xs text-slate-400">{metrics?.ssoUsers ?? 0} users</span></div>
          <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm"><span className="text-[10px] font-bold text-slate-500 uppercase">Active Sessions</span><p className="text-2xl font-black text-slate-900 mt-1">{activeSessions}</p><span className="text-xs text-slate-400">{highRiskSessions} high risk</span></div>
          <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm"><span className="text-[10px] font-bold text-slate-500 uppercase">MFA Coverage</span><p className="text-2xl font-black text-slate-900 mt-1">{metrics?.mfaEnrolled ?? '...'}</p><span className="text-xs text-slate-400">{metrics?.mfaEnforced ?? 0} enforced</span></div>
          <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm"><span className="text-[10px] font-bold text-slate-500 uppercase">Login Success</span><p className="text-2xl font-black text-slate-900 mt-1">{metrics?.loginSuccessRate ?? '...'}%</p><span className="text-xs text-slate-400">{metrics?.failedLogins24h ?? 0} failures (24h)</span></div>
        </div>
        ...
        <div className="flex items-center gap-1 bg-white p-1.5 rounded-xl border border-slate-200 shadow-sm overflow-x-auto">
          {VIEW_TABS.map(tab => (
            <button key={tab.id} onClick={() => setActiveView(tab.id)}
              ...
              {tab.icon} {tab.label}
              {tab.id === 'sessions' && activeSessions > 0 && <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-black ${activeView === tab.id ? 'bg-white/20 text-white' : 'bg-indigo-100 text-indigo-700'}`}>{activeSessions}</span>}
              {tab.id === 'audit' && <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-black ${activeView === tab.id ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-600'}`}>{auditLog.length}</span>}
            </button>
          ))}
        </div>
        ...
        {activeView === 'policies' && (
          <div className="space-y-3">
            {isLoadingPolicies ? Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-20 bg-white rounded-xl border border-slate-200 animate-pulse" />) :
            policies.map(p => (
              <div key={p.id} className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-xl ${p.enabled ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-500 border border-slate-200'}`}><Lock className="h-4 w-4" /></div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5"><h4 className="text-sm font-bold text-slate-800">{p.name}</h4><span className={`px-2 py-0.5 rounded text-[10px] font-black ${p.enabled ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>{p.enabled ? 'Enabled' : 'Disabled'}</span></div>
                    <p className="text-xs text-slate-500">{p.description}</p>
                    <div className="flex items-center gap-3 mt-1 text-[10px] text-slate-400"><span>{p.type.replace(/_/g, ' ')}</span><span>{p.triggerCount.toLocaleString()} triggers</span><span>{p.appliesTo.join(', ')}</span></div>
                  </div>
                </div>
              </div>
            ))}
