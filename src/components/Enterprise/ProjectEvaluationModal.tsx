  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-4xl shadow-2xl max-h-[92vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/60 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-1">
              <span className="font-mono text-xs text-slate-400 font-bold">{project.projectCode}</span>
            ...
            <div className="text-xs text-slate-500 flex items-center gap-2 mt-0.5">
              <span>{project.teamName}</span> • <span className="font-semibold text-blue-600 dark:text-blue-400">{project.college}</span>
            </div>
          </div>
        ...
        {/* Quick Links & KPI Bar */}
        <div className="px-6 py-3 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex flex-wrap items-center justify-between gap-4 text-xs">
          <div className="flex items-center gap-3">
            <a
              href={project.githubUrl}
              target="_blank"
              rel="noreferrer"
              className="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:text-blue-600 flex items-center gap-1.5 font-semibold"
            >
            ...
            {project.liveDeployUrl && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="px-3 py-1.5 rounded-lg bg-blue-50 dark:bg-blue-950 text-blue-600 font-semibold flex items-center gap-1.5"
              >
          ...
          <div className="flex items-center gap-2">
            <span className="text-slate-400 font-bold uppercase text-[10px]">Judge Score:</span>
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          {activeTab === 'rubric' && (
            <form onSubmit={handleSubmitReview} className="space-y-5">
              <div className="p-4 rounded-2xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-900 flex justify-between items-center text-xs">
                <div>
                  <span className="text-slate-400 font-bold uppercase text-[10px]">Calculated Composite Score</span>
                  <div className="text-2xl font-black text-blue-600 dark:text-blue-400 mt-0.5">
                    {calculateComposite()}% / 100%
                  </div>
                </div>
                <div className="text-right">
                  <span className="text-slate-400 font-bold uppercase text-[10px]">Recommendation</span>
                  <select
                    value={recommendation}
                    onChange={(e) => setRecommendation(e.target.value as any)}
                    className="block mt-1 p-2 rounded-lg bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-xs font-bold"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 space-y-2">
                  <div className="flex justify-between font-bold">
                    ...
                    <span className="text-blue-600">{scoreInnovation} / 10</span>
                  </div>
                  <input
                    type="range"
                    min="0"
                ...
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 space-y-2">
                  <div className="flex justify-between font-bold">
                    ...
                    <span className="text-blue-600">{scoreTechDepth} / 10</span>
                  </div>
                  <input
                    type="range"
                    min="0"
                ...
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 space-y-2">
                  <div className="flex justify-between font-bold">
                    ...
                    <span className="text-blue-600">{scoreCodeQuality} / 10</span>
                  </div>
                  <input
                    type="range"
                    min="0"
                ...
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 space-y-2">
                  <div className="flex justify-between font-bold">
                    ...
                    <span className="text-blue-600">{scoreDemo} / 10</span>
                  </div>
                  <input
                    type="range"
                    min="0"
          ...
          {activeTab === 'telemetry' && (
            <div className="space-y-4 text-xs">
              <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 space-y-2">
                <span className="text-slate-400 font-bold uppercase text-[10px]">Project Description</span>
                <p className="text-slate-800 dark:text-slate-200 leading-relaxed">{project.description}</p>
              </div>

              <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 space-y-2">
                <span className="text-slate-400 font-bold uppercase text-[10px]">Utilized Technology Stack</span>
                <div className="flex flex-wrap gap-2 pt-1">
                  {project.techStack.map((tech) => (
                    <span key={tech} className="px-3 py-1 rounded-lg bg-blue-50 dark:bg-blue-950 text-blue-600 font-bold border border-blue-200 dark:border-blue-900">
                      {tech}
                    </span>
                  ))}
                </div>
            ...
            <div className="space-y-4 text-xs">
              <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className={project.plagiarism.overallSimilarityIndex > 30 ? 'text-rose-600 font-bold text-sm' : 'text-emerald-600 font-bold text-sm'}>
          ...
          {activeTab === 'reviews' && (
            <div className="space-y-3 text-xs">
              {project.reviews.length === 0 ? (
                <p className="text-slate-400 italic">No judge reviews recorded yet.</p>
              ) : (
                project.reviews.map((rev) => (
                  <div key={rev.id} className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 space-y-2">
                    <div className="flex justify-between font-bold">
                      ...
                      <span className="text-blue-600 font-extrabold">{rev.compositeScore}%</span>
                    </div>
                    <p className="italic text-slate-700 dark:text-slate-300">"{rev.writtenCritique}"</p>
                  </div>
                ))
              )}
            </div>
