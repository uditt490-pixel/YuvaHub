...
const STAGES: { key: PipelineStage; label: string; color: string }[] = [
  { key: 'SOURCED', label: 'Sourced', color: 'border-slate-300 dark:border-slate-700 bg-slate-500/10' },
  { key: 'AI_SCREENED', label: 'AI Screened', color: 'border-blue-300 dark:border-blue-700 bg-blue-500/10' },
  { key: 'TECHNICAL_ASSESSMENT', label: 'Technical Assessment', color: 'border-purple-300 dark:border-purple-700 bg-purple-500/10' },
  { key: 'LEADERSHIP_ROUND', label: 'Leadership Loop', color: 'border-amber-300 dark:border-amber-700 bg-amber-500/10' },
  { key: 'OFFER_EXTENDED', label: 'Offer Extended', color: 'border-emerald-300 dark:border-emerald-700 bg-emerald-500/10' },
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span className="px-3 py-1 rounded-full bg-blue-50 dark:bg-blue-950 text-blue-600 dark:text-blue-400 text-[11px] font-bold uppercase tracking-widest flex items-center gap-1.5 border border-blue-200 dark:border-blue-900">
        ...
        <div className="p-4 rounded-2xl bg-blue-600 text-white flex flex-wrap items-center justify-between gap-4 shadow-lg shadow-blue-500/20">
          ...
          <div className="flex items-center gap-2 text-xs">
            <button
              ...
              className="px-3 py-1.5 bg-white text-blue-700 font-bold rounded-xl hover:bg-blue-50 transition-colors"
            >
              Batch to Tech Round
            </button>
            <button
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-slate-800">
                  <span className="text-xs font-extrabold uppercase tracking-wide text-slate-800 dark:text-slate-200">
                    {stg.label}
                  </span>
                  ...
                  {colCandidates.length === 0 ? (
                    ...
                    colCandidates.map((cand) => (
                      <div
                        ...
                        className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md hover:border-blue-500 dark:hover:border-blue-500 transition-all cursor-pointer space-y-3"
                      >
                        <div className="flex items-start justify-between gap-2">
                          ...
                          {cand.priority === 'CRITICAL_MATCH' && (
                            <span className="p-1 rounded-md bg-amber-500 text-white shrink-0" title="Critical Match">
                        ...
                        {/* Metrics Meter */}
                        <div className="p-2 rounded-xl bg-slate-50 dark:bg-slate-800/80 text-[10px] space-y-1">
                          <div className="flex justify-between font-bold">
                            <span className="text-slate-500">AI Composite Fit</span>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {cand.skills.slice(0, 2).map((s) => (
                            <span
                              key={s.name}
                              className="text-[9px] px-1.5 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-medium"
                            >
                              {s.name}
                            </span>
                          ))}
                    ...
                    >
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-[11px] text-slate-500">{c.college}</div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="p-4">
                        {c.priority === 'CRITICAL_MATCH' ? (
                          ...
                          <span className="text-slate-500 text-[11px]">{c.priority}</span>
                        )}
                      </td>
                      ...
                      <td className="p-4 text-right space-x-2">
                        <button
                          ...
                          className="px-3 py-1 rounded-lg bg-blue-50 dark:bg-blue-950 text-blue-600 dark:text-blue-400 font-bold hover:bg-blue-100 transition-colors"
                        >
                          Inspect
                        </button>
                        <button
                          onClick={() => setFastTrackTarget(c)}
                          className="px-3 py-1 rounded-lg bg-amber-50 dark:bg-amber-950 text-amber-600 dark:text-amber-400 font-bold hover:bg-amber-100 transition-colors"
                        >
                          Fast-Track
                        </button>
                      </td>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-4">
            ...
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 space-y-3">
              <div className="flex justify-between text-xs font-bold">
                ...
                <span className="text-emerald-600 dark:text-emerald-400">99.4% (Calibrated)</span>
              </div>
              <div className="flex justify-between text-xs font-bold">
                <span>False-Positive Shortlist Rate</span>
                <span className="text-blue-600 dark:text-blue-400">&lt; 0.8%</span>
              </div>
              <div className="flex justify-between text-xs font-bold">
                <span>Enterprise SLA Compliance</span>
                <span className="text-purple-600 dark:text-purple-400">100% Guaranteed</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-center">
              <div className="p-3 rounded-xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-900">
                <div className="text-lg font-black text-blue-600 dark:text-blue-400">6 Campuses</div>
                <div className="text-[10px] text-slate-500">Active Telemetry Pipes</div>
              </div>
              <div className="p-3 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-900">
                <div className="text-lg font-black text-emerald-600 dark:text-emerald-400">Zero-Trust</div>
                <div className="text-[10px] text-slate-500">Cryptographic Identity</div>
              </div>
            </div>
          </div>
        </div>
