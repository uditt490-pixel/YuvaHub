...
import { HeartPulse, Calendar, ShieldCheck, UserCheck, Activity } from 'lucide-react';

interface TimelineProps {
  checkIns: any[];
}

export const StudentMentalWellnessTimeline: React.FC<TimelineProps> = ({ checkIns }) => {
  return (
    <div className="bg-slate-900/90 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-xl font-bold text-white flex items-center gap-2">
            ...
            Campus Wellness & Tele-Counseling Telemetry Ledger
          </h3>
          <p className="text-xs text-slate-400 mt-1">
            Real-time student wellness check-ins, burnout evaluations, and counselor dispatch logs.
          </p>
        </div>
        <span className="bg-slate-800 text-slate-300 font-semibold text-xs px-3 py-1 rounded-full border border-slate-700">
          {checkIns.length} Logged Check-Ins
        </span>
      </div>
      ...
      {checkIns.length === 0 ? (
        <div className="text-center py-10 border border-dashed border-slate-800 rounded-2xl">
          ...
          <p className="text-slate-400 text-sm font-medium">No student wellness check-ins logged yet.</p>
        </div>
      ) : (
            ...
            >
              <div className="flex items-start gap-3.5">
                ...
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-[10px] font-extrabold px-2.5 py-0.5 rounded-full border uppercase bg-teal-500/10 text-teal-300 border-teal-500/30">
                      {item.sessionStatus}
                    </span>
                  </div>
                  <div className="text-xs text-slate-400 mt-1 flex flex-wrap items-center gap-x-3 gap-y-1">
                    ...
                    <span>•</span>
                    <span>Stressor: {item.primaryStressor}</span>
                    <span>•</span>
                    <span>Mood: {item.moodRating}/5</span>
                  </div>
                </div>
              </div>
              ...
              <div className="text-right self-end sm:self-center w-full sm:w-auto pt-3 sm:pt-0 border-t sm:border-t-0 border-slate-800/80">
                <div className="text-lg font-black text-teal-400">
                  Burnout: {item.burnoutScorePercent}%
                </div>
