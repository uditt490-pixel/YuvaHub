  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-4xl shadow-2xl max-h-[92vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2 mb-1">
                ...
                {candidate.priority === 'CRITICAL_MATCH' && (
                  <span className="text-xs px-2 py-0.5 rounded-full font-bold bg-amber-500 text-white flex items-center gap-1 shadow-sm">
              ...
              <div className="flex flex-wrap items-center gap-4 text-xs text-slate-600 dark:text-slate-400">
                <span className="flex items-center gap-1">
                  <GraduationCap className="w-3.5 h-3.5 text-blue-600" /> {candidate.college}
                </span>
                <span className="flex items-center gap-1">
                  <Briefcase className="w-3.5 h-3.5 text-purple-600" /> {candidate.targetRole}
                </span>
        ...
        {/* Action Toolbar */}
        <div className="px-6 py-3 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-2 text-xs">
            ...
            <select
              ...
              className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-1.5 font-bold text-slate-800 dark:text-slate-200 text-xs focus:ring-2 focus:ring-blue-500 outline-none"
            >
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-50 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 text-xs font-semibold border border-blue-100 dark:border-blue-900">
                  ...
                  {candidate.skills.map((skill) => (
                    ...
                    >
                      <div className="flex items-center justify-between text-xs font-bold text-slate-800 dark:text-slate-200 mb-1.5">
                        ...
                        <span className="text-blue-600 dark:text-blue-400">{skill.score}%</span>
                      </div>
                ...
                {candidate.telemetry.map((log) => (
                  <div key={log.id} className="relative pl-7 space-y-1">
                    ...
                    <div className="text-xs text-slate-600 dark:text-slate-400">
                      By <span className="font-semibold text-blue-600 dark:text-blue-400">{log.actor}</span> ({log.actorRole})
                    </div>
                    {log.notes && (
                      <div className="p-2 rounded-lg bg-slate-50 dark:bg-slate-800 text-xs text-slate-700 dark:text-slate-300 italic border border-slate-200 dark:border-slate-700">
                        "{log.notes}"
                      </div>
                    )}
                  </div>
            ...
            <div className="space-y-6">
              <form onSubmit={handleAddNoteSubmit} className="space-y-3">
                ...
                <textarea
                  ...
                  className="w-full p-3 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-xs text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
                />
                <button
                  type="submit"
              ...
              <div className="space-y-3">
                ...
                {candidate.notes.length === 0 ? (
                  ...
                  candidate.notes.map((n, i) => (
                    <div
                      key={i}
                      className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs text-slate-800 dark:text-slate-200 leading-relaxed"
                    >
                      {n}
                    </div>
                  ))
              ...
              <div className="p-6 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                    <span className="text-xs text-slate-500">Expected Base CTC</span>
                    <div className="text-xl font-extrabold text-slate-900 dark:text-white mt-1">
                      ₹{candidate.expectedCtcLpa} LPA
                    </div>
                  </div>
                  <div className="p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                    <span className="text-xs text-slate-500">Joining Bonus</span>
                    <div className="text-xl font-extrabold text-emerald-600 dark:text-emerald-400 mt-1">
                      ₹4,00,000
                    </div>
                  </div>
                  <div className="p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                    <span className="text-xs text-slate-500">Stock Grants (ESOPs)</span>
