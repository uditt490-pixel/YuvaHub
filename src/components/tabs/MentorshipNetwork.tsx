  ...
  const DiscoverTab = () => (
    <div className="space-y-6">
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400"/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search mentors, skills, companies..." className="bg-white/5 border border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-white text-sm w-64 focus:outline-none focus:border-cyan-500/50"/></div>
        <div className="flex gap-1 flex-wrap">
          <button onClick={()=>setSpec('all')} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${spec==='all'?'bg-white/10 text-white border border-white/20':'bg-white/5 text-gray-400 border border-white/10'}`}>All</button>
          {SPECIALTIES.map(s=>(<button key={s.id} onClick={()=>setSpec(s.id)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${spec===s.id?`${s.color} border border-current/40`:'bg-white/5 text-gray-400 border border-white/10'}`}>{s.label}</button>))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filtered.map(m=>{
          return(
          <div key={m.id} onClick={()=>setSel(sel===m.id?null:m.id)} className={`bg-white/5 backdrop-blur-md border rounded-2xl p-5 cursor-pointer transition-all ${sel===m.id?'border-cyan-500/50 ring-1 ring-cyan-500/30':'border-white/10 hover:border-white/20'}`}>
            ...
            <div className="flex flex-wrap gap-1 mb-3">
              {m.skills.slice(0,5).map(s=><span key={s} className="px-2 py-0.5 rounded text-[9px] bg-white/5 text-gray-400">{s}</span>)}
              {m.skills.length>5&&<span className="text-[9px] text-gray-500">+{m.skills.length-5}</span>}
            </div>
            <div className="grid grid-cols-3 gap-2 mb-3">
              <div className="bg-white/5 rounded-lg p-2 text-center"><div className="text-white text-xs font-medium">{m.mentees}</div><div className="text-[9px] text-gray-500">Mentees</div></div>
              <div className="bg-white/5 rounded-lg p-2 text-center"><div className="text-white text-xs font-medium">{m.sessions}</div><div className="text-[9px] text-gray-500">Sessions</div></div>
              <div className="bg-white/5 rounded-lg p-2 text-center"><div className="text-white text-xs font-medium">{m.responseTime}</div><div className="text-[9px] text-gray-500">Response</div></div>
            </div>
            {sel===m.id&&(
              <div className="mt-3 pt-3 border-t border-white/10 space-y-3">
                ...
                <div>
                  ...
                  <div className="flex gap-1">{['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map(d=><span key={d} className={`px-2 py-0.5 rounded text-[9px] ${m.availability.includes(d)?'bg-emerald-500/20 text-emerald-400':'bg-white/5 text-gray-600'}`}>{d}</span>)}</div>
                </div>
                <div>
            ...
            <div className="flex items-center justify-between mt-3 pt-3 border-t border-white/5">
              <button onClick={e=>{e.stopPropagation();toggleBookmark(m.id)}} className={`px-3 py-1.5 rounded-lg text-[10px] font-medium border transition-all ${bookmarks.has(m.id)?'bg-amber-500/20 border-amber-500/40 text-amber-400':'bg-white/5 border-white/10 text-gray-400'}`}>{bookmarks.has(m.id)?'★ Saved':'Save'}</button>
          ...
          <div className="space-y-3">{upcoming.map(s=>(
            <div key={s.id} className="bg-cyan-500/5 border border-cyan-500/20 rounded-2xl p-4 flex items-center gap-4">
              ...
              <div className="flex gap-2">
                ...
                <button className="px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg text-gray-400 text-[10px]">Reschedule</button>
              </div>
            </div>))}</div>
        </div>
      )}
        ...
        <div>
          ...
          <div className="space-y-3">{completed.map(s=>(
            <div key={s.id} className="bg-white/5 border border-white/10 rounded-2xl p-4">
              ...
              {s.notes&&<div className="text-gray-400 text-xs mb-2 bg-white/5 rounded-lg p-2">{s.notes}</div>}
  ...
  const MatchesTab = () => (
    <div className="space-y-6">
      ...
      {MATCHES.map((match,i)=>{
        ...
        return(
        <div key={match.mentorId} className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-5">
          ...
          <div className="flex justify-end gap-2">
            <button className="px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-gray-400 text-xs hover:text-white transition-all">View Profile</button>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6">
        ...
        <div className="space-y-3">{MENTORS.filter(m=>SESSIONS.some(s=>s.mentorId===m.id)).map(m=>{
          ...
          return(<div key={m.id} className="flex items-center gap-4"><div className="w-28 text-xs text-gray-300 truncate">{m.name}</div><div className="flex-1 h-4 bg-white/10 rounded-full overflow-hidden"><div className="h-full bg-cyan-500 rounded-full" style={{width:`${(count/3)*100}%`}}/></div><div className="w-8 text-right text-xs text-gray-400">{count}</div></div>);
        })}</div>
      </div>
      <div className="bg-gradient-to-r from-emerald-500/10 to-cyan-500/10 border border-emerald-500/20 rounded-2xl p-6">
        ...
        <div className="space-y-2">
          <div className="text-sm text-gray-300 bg-white/5 rounded-xl p-3">🎯 Your best match is Priya Sharma (95%) — book a session to maximize learning</div>
          <div className="text-sm text-gray-300 bg-white/5 rounded-xl p-3">📈 Video sessions have 20% higher satisfaction than chat — prefer video calls</div>
          <div className="text-sm text-gray-300 bg-white/5 rounded-xl p-3">⏰ Your peak learning time is morning (9-11 AM) — schedule important sessions then</div>
        </div>
      </div>
    </div>);
  };
